...
4. When the alternator signal on `D2` goes HIGH (engine running), runs a 5-second after-glow — **only** if the boot-time reading was ≥ 600 ADC ("cold"). Otherwise the relay drops the moment pre-glow ends.
...
After-glow is a 5-second constant, gated on boot A0 ≥ 600. Both the threshold and the duration are firmware constants, not exposed in the UI.
