...
3. **Cold-only after-glow** — if the boot-time [[temperature-reading]] was ≥ 700 ADC, hold the [[glow-relay]] for 3 seconds after the alternator goes high; otherwise skip after-glow entirely and de-energise the relay the moment pre-glow ends.
...
2. **No after-glow tuning** — the 3-second duration and ≥ 700 ADC cold threshold are firmware constants, not exposed in the web UI.
...
| Name | Value | Purpose |
|---|---|---|
| `AFTER_GLOW_DURATION_MS` | `3000` | Length of after-glow when it runs (3 s) |
| `AFTER_GLOW_COLD_THRESHOLD` | `700` | After-glow runs only if `bootTempReading >= 700` |
...
3. **Cold-only after-glow fires for 3 s.** With `A0` ≥ 700 ADC at boot: after pre-glow finishes, pulling `D2` HIGH causes `GLOW_RELAY` to stay HIGH for exactly 3 s, then drop LOW. With `A0` < 700 ADC at boot: pulling `D2` HIGH drops `GLOW_RELAY` LOW *immediately* — no after-glow.
